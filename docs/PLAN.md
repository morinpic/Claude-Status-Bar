...
| トリガー | 条件 | メッセージ例 |
|---------|------|-------------|
| 障害発生 | `indicator` が `none` → `minor/major/critical` | ⚠ Incident detected on Claude: Elevated errors on Claude API |
| 障害復旧 | `indicator` が `minor/major/critical` → `none` | ✅ Claude is back to operational |

通知メッセージはアプリ内言語設定に従ってローカライズされる（英語 / 日本語）。
